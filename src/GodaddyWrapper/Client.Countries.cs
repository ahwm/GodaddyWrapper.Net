...
namespace GodaddyWrapper
{
    public partial class GoDaddyClient
    {
        ...
        public async Task<CountrySummaryResponse> RetrieveCountries(CountriesRetrieve request)
        {
            ...
            var response = await httpClient.GetAsync($"v1/countries{QueryStringBuilder.RequestObjectToQueryString(request)}");
        ...
        {
            CheckRequestValid(request);
            var response = await httpClient.GetAsync($"v1/countries/{CountryKey}{QueryStringBuilder.RequestObjectToQueryString(request)}");
            await CheckResponseMessageIsValid(response);
