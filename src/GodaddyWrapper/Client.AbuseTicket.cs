...
namespace GodaddyWrapper
{
    public partial class GoDaddyClient
    {
        ...
        {
            var response = await httpClient.PostAsJsonAsync($"v1/abuse/tickets", request, JsonSettings);
        ...
        {
            var response = await httpClient.GetAsync($"v1/abuse/tickets{QueryStringBuilder.RequestObjectToQueryString(request)}");
            await CheckResponseMessageIsValid(response);
        ...
        {
            var response = await httpClient.GetAsync($"v1/abuse/tickets/{request.TicketId}");
            await CheckResponseMessageIsValid(response);
