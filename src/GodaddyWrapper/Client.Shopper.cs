...
namespace GodaddyWrapper
{
    public partial class GoDaddyClient
    {
        ...
        public async Task<ShopperIdResponse> CreateSubaccount(SubaccountCreate request)
        {
            ...
            var response = await httpClient.PostAsJsonAsync($"v1/shoppers/subaccount", request, JsonSettings);
        ...
        {
            CheckRequestValid(request);
            var response = await httpClient.GetAsync($"v1/shoppers/{request.ShopperId}");
            await CheckResponseMessageIsValid(response);
        ...
        {
            CheckRequestValid(request);
            var response = await httpClient.PostAsJsonAsync($"v1/shoppers/{shopperId}", request, JsonSettings);
            await CheckResponseMessageIsValid(response);
            return await response.Content.ReadAsAsync<ShopperIdResponse>(JsonSettings);
        }
    }
